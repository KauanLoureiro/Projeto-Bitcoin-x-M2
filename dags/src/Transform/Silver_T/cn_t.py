#%%
import pandas as pd
from io import StringIO
import json
from airflow.providers.postgres.hooks.postgres import PostgresHook
from pathlib import Path

def clean_string(x):
    if isinstance(x, str):
        return x.replace("\xa0", "").strip()
    return x
    
# def format(df):
#     df = pd.DataFrame(df)
#     df = df.T
#     df = df[[4,6]]
#     df.index = range(0 , len(df))
#     df = df.drop([0,1,2])
#     df = df.rename(columns={
#         4:"Date",
#         6:"M2_cn"
#     })
#     df["Date"] = df["Date"].apply(clean_string)
#     df["M2_cn"] = df["M2_cn"].apply(clean_string)
#     df["Date"] = pd.to_datetime(df["Date"].astype(str))
#     df.loc[12,"Date"] = df.loc[12,"Date"].replace(month=10)
#     df = df.sort_values(by="Date", ascending=False)
#     return df

def format(df):
    df = pd.DataFrame(df)
    df = df.T
    df = df[[4,6]]
    df.index = range(0 , len(df))

    df = df.rename(columns={
        4:"Date",
        6:"M2_cn"
    })

    # limpar strings
    df["Date"] = df["Date"].apply(clean_string)
    df["M2_cn"] = df["M2_cn"].apply(clean_string)

    # manter só datas válidas
    df = df[df["Date"].astype(str).str.contains(r"\d{4}\.\d{2}", na=False)]

    # conversões seguras
    df["Date"] = pd.to_datetime(df["Date"], format="%Y.%m", errors="coerce")
    df["M2_cn"] = pd.to_numeric(df["M2_cn"], errors="coerce")

    # remover lixo
    df.dropna(subset=["Date", "M2_cn"], inplace=True)

    df = df.sort_values(by="Date", ascending=False)

    return df

def extract_cn_silver():
    bronze_path = Path("/opt/airflow/data/Bronze")
    cn_path = bronze_path / "M2_cn.json"

    with open(cn_path, "r", encoding="utf-8") as f:
        cn = json.load(f)
    return cn

def transform_cn_silver(cn):
    cn_format = []

    for i in cn:
        cn_format.append(format(i))

    M2_cn = pd.concat(cn_format, axis=0, ignore_index=True)
    M2_cn.dropna(inplace=True)
    M2_cn["M2_cn"] = pd.to_numeric(M2_cn["M2_cn"])*10**9

    M2_cn["YearMonth"] = M2_cn["Date"].dt.to_period("M").astype(str)

    M2_cn = M2_cn.groupby("YearMonth", as_index=False).agg({
        "M2_cn": "mean"
    })
    M2_cn = M2_cn.sort_values("YearMonth", ascending=False)
    return M2_cn

def load_cn_silver(M2_cn):
    #Jogando para a memoria para fzr o copy
    buffer = StringIO()
    M2_cn.to_csv(buffer, index=False, header=True)
    buffer.seek(0)

    #Mexendo no DB
    hook = PostgresHook(postgres_conn_id="postgres_dw")
    conn = hook.get_conn()
    cur = conn.cursor()

    cur.execute("CREATE SCHEMA IF NOT EXISTS silver;")

    cur.execute(""" 
                CREATE TABLE IF NOT EXISTS silver.m2_cn_cny(
                yearmonth TEXT,
                M2_cn FLOAT
                ); 
                """)

    cur.execute("TRUNCATE TABLE silver.m2_cn_cny;")

    cur.copy_expert(
        sql="COPY silver.m2_cn_cny FROM STDIN WITH CSV HEADER",
        file=buffer
    )

    conn.commit()
    cur.close()
    conn.close()